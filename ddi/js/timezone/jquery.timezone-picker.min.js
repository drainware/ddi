...
jpalanco: IMPORTANT! I've been changing $pin by $(".timezone-pin") 
...
methods.init = function(initOpts) {
  ...
  return $origCall.each(function(index, item) {
    ...
    $(mapElement).find('area').click(function() {
      ...
      if ($(".timezone-pin")) {
        $(".timezone-pin").css('display', 'block');
        var pinCoords = $(areaElement).attr('data-pin').split(',');
        var pinWidth = parseInt($(".timezone-pin").width() / 2);
        var pinHeight = $(".timezone-pin").height();

	var left = pinCoords[0] - pinWidth;
	var top = pinCoords[1] - pinHeight;
