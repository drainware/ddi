...
<h2>{$translations[$t_application_creation_process]}</h2>
<div class="assistant-wrapper">
    <form id="form-assistant" class="wizard_form" method="post" action="?module=atp&action=createApp">
        <fieldset class="dwstep">
            <legend>{$translations[$t_description]}</legend> 
            <p>
                <span class="entitle_medium">{$translations[$t_application]}:</span>
                <input type="text" class="text medium required" name="app_name" value="" />
            </p>
            <p>
                <span class="entitle_medium">{$translations[$t_short_desc]}:</span>
                <input type="text" class="text medium required" name="app_description" value="" />
            </p>
        </fieldset>
        <fieldset class="dwstep">
            ...
            {foreach from = $atp_variables key=k_variable item=i_variable}
                <p>
                    ...
                    -->
                    <input type="text" class="text medium" name="app_vars[{$k_variable}][value]" value="" {if $k_variable eq "SearchString"} placeholder="Hex. ej: 90,C0,FF" {/if} />
                </p>
            {/foreach}
            <p>
        ...
        <fieldset class="dwstep">
            ...
            <p>
                ...
                <input type="text" class="text medium" name="app_extensions" id="app_extension" value="" />
            </p>
        </fieldset>
        <fieldset class="dwstep">
