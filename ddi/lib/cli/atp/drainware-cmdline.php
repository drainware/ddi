...
function analysePDF($file = array(), $sample_id = 0) {
	global $PDFstringSearch, $PDFhexSearch, $global_block_encoding, $global_engine, $pdfdir, $global_store_files, $global_export_all;

	$md5 = $file['md5'];

	logDebug($file['md5']." start procesing");

	$file_raw = file_get_contents($file['filename']);
	$result = pdfSlice($file_raw);
	logDebug($file['md5']." end procesing");


	$fileUpdate = array('exploit' => 0, 'hits' => 0, 'completed' => 1, 'is_malware' => 0, 'summary' => '', 'severity' => 0);


	$header = file_get_contents($file['filename'], NULL, NULL, 0, 1024);
	if (!preg_match("/%PDF/si", $header)) {
		fputs($fp,  "File missing PDF signature - not processed.\n");
...
function parseObjStm($params, $stream) {
	...
	if (preg_match("/(#46|F)(#69|i)(#72|r)(#73|s)(#74|t)\s+(\d+)/s", $params, $res2) ) {
		...
		$first = $res2[6];
		//fputs($fp,  "First=$first\n");
	}

	$header = substr($stream, 0, $first-1);
	//fputs($fp,  "Header=$header\n");
	...
	if(isset($resh[1]) ) {
		for ($i=0; $i < count($resh[1]); $i++) {
			//fputs($fp,  "Obj=".$resh[1][$i]." loc=".($resh[2][$i]+$first));
			if ($i+1 >= count($resh[1])) {
				$end = strlen($stream);
				//fputs($fp,  " End=".$end);
			} else {
